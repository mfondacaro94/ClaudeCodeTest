...
Same architecture as MLB/UFC predictors, adapted for tennis match outcomes.
...
def load_data():
    ...
    # Linear sample weights (recent matches weighted more)
...
def evaluate_ensemble(models: list, X_test, y_test) -> dict:
    ...
    metrics = {
        ...
        "n_test": len(y_test),
        "n_models": len(models),
        "p1_win_rate_actual": round(y_test.mean(), 4),
        "p1_win_rate_predicted": round(ensemble_prob.mean(), 4),
    }
